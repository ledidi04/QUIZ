...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .navbar { display: flex; justify-content: space-between; align-items: center; background: rgba(255,255,255,0.95); padding: 0 2rem; height: 70px; position: sticky; top: 0; z-index: 1000; border-bottom: 1px solid var(--gray-200); }
        ...
        @media (max-width: 768px) {
            ...
            .nav-menu { display: none; flex-direction: column; position: absolute; top: 60px; left: 0; width: 100%; background: var(--white); padding: 1rem; border-bottom: 1px solid var(--gray-200); z-index: 999; }
...
<body>
    ...
    <script>
        ...
        function renderQuestion() {
            ...
            h += `<p class="question-text">${q.enonce}</p>`;
            
            if (q.type === 'qcm') {
                (q.options||[]).forEach((o,i) => h += `<button class="option-btn" data-correct="${o.correct}">${String.fromCharCode(65+i)}. ${o.texte}</button>`);
        ...
        function handleAnswer(q, ok) {
            if(ok) score++;
            if(q.explication) { const d = document.createElement('div'); d.className='explication'; d.innerHTML='💡 '+q.explication; document.getElementById('quizContainer').appendChild(d); }
